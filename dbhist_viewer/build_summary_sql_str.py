#--
# Construct sql syntax to get summary data from DBA_HIST_SYSMETRIC_SUMMARY
...
def build_summary_sql(rac_instances, selected_instance, metric_list, browzer_tz_name,db_os_tz):
    ...
    for metric in metric_list:
        ...
        for inst in inst_range:
            str2_instance_list.append('nvl(f'+str(m_cnt)+str(inst+1)+('*TPS'+str(inst+1) if 'Derived' in metric else '') +',0)')
...
if __name__ == '__main__':
    ...
    v_sql = build_summary_sql(rac_instances, selected_instance, metric_list)
    ...
    selected_instance=1
    v_sql = build_summary_sql(rac_instances, selected_instance, metric_list)
    print("selected_instance"+str(selected_instance)+' :',v_sql)
